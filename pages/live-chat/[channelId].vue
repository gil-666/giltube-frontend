<template>
    <div class="fixed inset-0 bg-zinc-950 text-white z-50 live-chat">
        <div class="h-screen w-screen bg-zinc-900 flex flex-col">
            <div class="p-3 border-b border-zinc-800 flex items-center justify-between gap-2 bg-zinc-900 sticky top-0">
                <div class="min-w-0">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <NuxtLink :to="streamPath" target="_blank" rel="noopener"
                        class="px-2 py-1 text-xs bg-zinc-800 hover:bg-zinc-700 rounded transition">
                        Open stream
                    </NuxtLink>
                    <button @click="refreshChat" class="px-2 py-1 text-xs bg-zinc-800 hover:bg-zinc-700 rounded transition">
                        Refresh
                    </button>
                </div>
            </div>
            ...
            <div class="p-3 border-t border-zinc-800 space-y-2">
                ...
                <div v-if="!isLive" class="text-xs text-yellow-300 bg-yellow-900/30 border border-yellow-700 rounded p-2">
                ...
                <div v-else-if="availableChannels.length === 0" class="text-xs text-gray-300 bg-zinc-800 rounded p-2">
...
// Disable default layout to completely ignore header
...
const streamPath = computed(() => {
    ...
    return `/live/${encodeURIComponent(channelId.value)}`
})

useMetaTags({
    title: `${channelName.value} Live Chat - GilTube`,
    description: 'Pop-out live chat on GilTube.'
})
...
const refreshChat = async () => {
    ...
    try {
        const messages = await getLiveChatMessages(channelId.value, 120)
